...
{
	QMessageBox::information(NULL, "title", "connect clicked");
	// 新建tcpsocket接口
	...
	//connect(visualTcpSocket, SIGNAL(readChannelFinished()), this, SLOT());
...
{
	QMessageBox::information(NULL, "title", "startsever clicked");
	// 测试QProcess功能
	QProcess p(0);
	p.start("adb devices");
	p.waitForStarted();
